...
const func = async () => {
	...
	await nosql.insertMany((await insertWeatherCSV()));
    
    const deleteQuery = JSON.parse("{ }")
	
	var result = (await nosql.deleteMany(deleteQuery)).deletedCount // => TEST PASSED! Delete instance inside DB and return number of instance eliminated.
	
	if(result > 0)
		console.log("One or more item(s) was eliminated") 
